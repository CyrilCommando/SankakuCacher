...
So as it turns out Firefox & Chrome are total messes. Downloading full videos & gifs in history menu doesn't work in firefox if the file is larger than 32mb due to an extremely obscure issue where the IPC message size is limited to 32mb. (This actually doesn't apply anymore since I use IndexedDB now to save the images, but it's still true for the localstorage API!) Wild stuff out here.
