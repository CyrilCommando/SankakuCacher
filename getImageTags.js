function getImageTags(docu)
{
  ...
  charactertags.forEach(element => {
    tag_array.push ({"type": "character_tag", "tag": $(element).find("div > a").text().slice (0)})
  });
  seriestags.forEach(element => {
    tag_array.push ({"type": "copyright_tag", "tag": $(element).find("div > a").text().slice (0)})
  });
  artisttags.forEach(element => {
    tag_array.push ({"type": "artist_tag", "tag": $(element).find("div > a").text().slice (0)})
  });
  genretags.forEach(element => {
    tag_array.push ({"type": "genre_tag", "tag": $(element).find("div > a").text().slice (0)})
  });
  studiotags.forEach(element => {
    tag_array.push ({"type": "studio_tag", "tag": $(element).find("div > a").text().slice (0)})
  });
  metadatatags.forEach(element => {
    // if ($(element).find("a").text().slice (0, -1).match("^animated"))
    // {
      tag_array.push ({"type": "metadata_tag", "tag": $(element).find("div > a").text().slice (0)})
    // }
  });
  generaltags.forEach(element => {
    tag_array.push ({"type": "general_tag", "tag": $(element).find("div > a").text().slice (0)})
  });
